...
@section('title', 'Edit Sub Data - Lab Digital FEB UNDIP')
...
@section('content')
    ...
    <div class="bg-white rounded-2xl shadow-lg p-6 max-w-2xl">
        <h1 class="text-xl font-bold text-gray-800 mb-2">Edit Sub Data</h1>
        <p class="text-gray-500 mb-6">Kategori: <strong>{{ $master->name }}</strong></p>

        <form action="{{ route('admin.bps.sub-data.update', [$master, $subDatum]) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="space-y-4">
                <div>
                    ...
                    <input type="text" id="name" name="name" value="{{ old('name', $subDatum->name) }}" required
                ...
                <div>
                    ...
                    <input type="text" id="code" name="code" value="{{ old('code', $subDatum->code) }}"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500">
                ...
                <div>
                    ...
                    <textarea id="description" name="description" rows="3"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500">{{ old('description', $subDatum->description) }}</textarea>
                    @error('description')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
                </div>

                <div>
                    <label for="sort_order" class="block text-sm font-semibold text-gray-700 mb-2">Urutan</label>
                    <input type="number" id="sort_order" name="sort_order" value="{{ old('sort_order', $subDatum->sort_order) }}" min="0"
                        class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 focus:border-teal-500">
                ...
                <div>
                    <label class="flex items-center">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $subDatum->is_active) ? 'checked' : '' }}
