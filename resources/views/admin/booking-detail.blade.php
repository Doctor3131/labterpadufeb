...
<body class="bg-gray-50">
        ...
        <div class="container mx-auto px-4 md:px-6 py-3 md:py-4">
            <div class="flex justify-between items-center">
                <div class="flex items-center space-x-2 md:space-x-3">
                    <img src="{{ asset('images/LogoUndips.png') }}" alt="Logo Undip" class="h-8 md:h-16 w-auto object-contain">
                </div>
                <a href="{{ route('admin.dashboard') }}" class="text-gray-600 hover:text-yellow-600 text-sm md:text-base flex items-center">
    ...
    <div class="container mx-auto px-4 md:px-6 py-4 md:py-8 max-w-4xl">
        <!-- Header -->
        <div class="bg-white rounded-lg shadow-lg p-4 md:p-6 mb-4 md:mb-6">
            <div class="flex flex-col md:flex-row md:justify-between md:items-start gap-4 mb-4">
                <div class="flex-1">
                    <h1 class="text-2xl md:text-3xl font-bold text-gray-800 mb-3">Detail Peminjaman</h1>
                    <div class="flex flex-wrap items-center gap-2">
                        @if($booking->status === 'pending')
                            <span class="px-3 py-1.5 bg-yellow-100 text-yellow-800 text-xs md:text-sm font-semibold rounded-lg">⏳ Menunggu Persetujuan</span>
                        @elseif($booking->status === 'approved')
                            <span class="px-3 py-1.5 bg-green-100 text-green-800 text-xs md:text-sm font-semibold rounded-lg">✓ Disetujui</span>
                        @else
                            <span class="px-3 py-1.5 bg-red-100 text-red-800 text-xs md:text-sm font-semibold rounded-lg">✗ Ditolak</span>
                        @endif
                ...
                @if($booking->status === 'pending')
                    <div class="flex flex-col sm:flex-row gap-2 w-full md:w-auto">
                        <form action="{{ route('admin.booking.approve', $booking->id) }}" method="POST" class="flex-1 sm:flex-none">
                            @csrf
                            <button type="submit" onclick="return confirm('Setujui peminjaman ini?')" class="w-full px-4 md:px-6 py-2 bg-green-500 hover:bg-green-600 text-white font-semibold rounded-lg transition-all">
                                Setujui
                            </button>
                        </form>
                        <button onclick="showRejectModal()" class="flex-1 sm:flex-none px-4 md:px-6 py-2 bg-red-500 hover:bg-red-600 text-white font-semibold rounded-lg transition-all">
                            Tolak
                        </button>
                    </div>
                @endif
        ...
        <div class="bg-white rounded-lg shadow-lg p-4 md:p-6 mb-4 md:mb-6">
            <h2 class="text-lg md:text-xl font-bold text-gray-800 mb-4">Informasi Laboratorium & Waktu</h2>
            <div class="space-y-4">
                <div>
                    <label class="block text-sm font-semibold text-gray-600 mb-1">Laboratorium</label>
                    <p class="text-lg md:text-xl font-semibold text-yellow-600">{{ $booking->lab->name }}</p>
                ...
                <div>
                    ...
                    <p class="text-base md:text-lg">{{ $booking->participant_count }} orang</p>
                </div>
            </div>
        </div>
        ...
        <!-- Informasi Peminjam -->
        <div class="bg-white rounded-lg shadow-lg p-4 md:p-6 mb-4 md:mb-6">
            <h2 class="text-lg md:text-xl font-bold text-gray-800 mb-4">Informasi Peminjam</h2>
            <div class="space-y-3">
                <div>
        ...
        <!-- Detail Kegiatan -->
        <div class="bg-white rounded-lg shadow-lg p-4 md:p-6 mb-4 md:mb-6">
            <h2 class="text-lg md:text-xl font-bold text-gray-800 mb-4">Detail Kegiatan</h2>
            ...
            @if($booking->booking_type === 'non_perkuliahan')
                <div class="space-y-3">
                    ...
                    <div>
                        ...
                        <p class="text-base md:text-lg font-semibold text-gray-800">{{ $booking->activity_name }}</p>
                    </div>
                    <div>
            ...
            @else
                <div class="space-y-3">
                    ...
                    <div>
                        ...
                        <p class="text-base md:text-lg font-semibold text-gray-800">{{ $booking->course_name }}</p>
                    </div>
                    <div>
        ...
        @if($booking->document_path)
            <div class="bg-white rounded-lg shadow-lg p-4 md:p-6 mb-4 md:mb-6">
                <h2 class="text-lg md:text-xl font-bold text-gray-800 mb-4">Dokumen Pendukung</h2>
                <a href="{{ asset('storage/' . $booking->document_path) }}" target="_blank" class="inline-flex items-center px-4 py-2.5 bg-blue-500 hover:bg-blue-600 text-white rounded-lg transition-all w-full sm:w-auto justify-center">
        ...
        @if($booking->status === 'rejected' && $booking->rejection_reason)
            <div class="bg-red-50 border border-red-200 rounded-lg p-4 md:p-6 mb-4 md:mb-6">
                <h2 class="text-lg md:text-xl font-bold text-red-800 mb-2">Alasan Penolakan</h2>
                <p class="text-red-700">{{ $booking->rejection_reason }}</p>
            </div>
        @endif

        <!-- Metadata -->
        <div class="bg-gray-100 rounded-lg p-4 text-xs md:text-sm text-gray-600 space-y-1">
    ...
    <!-- Reject Modal -->
    <div id="rejectModal" class="fixed inset-0 bg-black/60 backdrop-blur-sm hidden items-center justify-center z-50 px-4">
            ...
            <div class="flex items-start justify-between mb-6">
                <div class="flex items-start space-x-3 flex-1">
                    ...
                    <div class="flex-1">
                        ...
                        <p class="text-sm text-gray-500 mt-1">Berikan alasan penolakan</p>
                    </div>
                </div>
