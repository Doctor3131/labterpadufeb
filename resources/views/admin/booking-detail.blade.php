                    ...
                    <div class="p-5">
                         <div class="flex flex-col gap-4 p-4 bg-slate-50 rounded-xl border border-slate-200 group hover:border-yellow-200 transition-colors sm:flex-row sm:items-center sm:justify-between">
                            ...
                            <a href="{{ route('admin.secure-file', ['path' => $booking->document_path]) }}" target="_blank" rel="noopener" class="px-4 py-2 bg-white hover:bg-yellow-100 text-yellow-800 text-sm font-semibold rounded-lg border border-yellow-300 transition-all flex items-center justify-center">
                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 3h7m0 0v7m0-7L10 14"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 5v14h14v-5"></path></svg>
                                Buka di tab baru
                            </a>
                        </div>
                        <iframe src="{{ route('admin.secure-file', ['path' => $booking->document_path]) }}" title="Pratinjau dokumen peminjaman #{{ $booking->id }}" class="mt-4 h-[30rem] w-full rounded-xl border border-gray-200 bg-white" loading="lazy">
                            Dokumen tidak dapat ditampilkan di peramban ini. Gunakan tombol “Buka di tab baru”.
                        </iframe>
                    </div>
                </div>
                @endif
